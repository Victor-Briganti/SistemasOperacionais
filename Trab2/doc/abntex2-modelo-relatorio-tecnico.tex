...
\begin{itemize}
    ...
    \item \textbf{Métodos Privados:}
        \begin{itemize}
            ...
            \begin{itemize}
                \item \texttt{Parâmetro}: \textit{from} - Caminho do arquivo interno.
                \item \texttt{Parâmetro}: \textit{to} - Caminho para o arquivo externo.
                \item \texttt{Exceção}: Gera uma exceção se houver algum problema durante a busca do cluster.
                \item \texttt{Retorno}: \textit{true} se foi possível copiar, \textit{false} caso contrário.
            \end{itemize}
            \item \texttt{copyExternalData}: Copia dados externos para uma localização interna.
            \begin{itemize}
                \item \texttt{Parâmetro}: \textit{from} - Caminho do arquivo externo.
                \item \texttt{Parâmetro}: \textit{to} - Caminho para o arquivo interno.
                \item \texttt{Exceção}: Gera uma exceção se houver algum problema durante a busca do cluster.
                \item \texttt{Retorno}: \textit{true} se foi possível copiar, \textit{false} caso contrário.
            \end{itemize}
            \item \texttt{copyInSystemData}: Copia dados de um caminho interno para outro.
            \begin{itemize}
                \item \texttt{Parâmetro}: \textit{from} - Caminho do arquivo interno.
                \item \texttt{Parâmetro}: \textit{to} - Caminho para o arquivo interno.
                \item \texttt{Exceção}: Gera uma exceção se houver algum problema durante a busca do cluster.
